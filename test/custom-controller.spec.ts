...
describe("PrometheusModule with a custom controller (register)", function () {
  let agent: Agent;
  let app: App;
...
describe("PrometheusModule with a custom controller (registerAsync)", function () {
  let agent: Agent;
  let app: App;
  let fake: sinon.SinonSpy;
  ...
  beforeEach(async function () {
    ...
    ({ agent, app } = await createAsyncPrometheusModule({
      useFactory: () => ({ controller: CustomController }),
    }));
  });
